data {
    ...
    int<lower=0> w[n_obs];  // fitness (recruits)
  }
...
// Create vector of predicted values
 ...
 // Random effects prior distribution
   ...
   to_vector(sigma_isl) ~ normal(0, 1);
    ...
    // Likelihood function
     w~neg_binomial_2(exp(v), phi); // sampling statement. How do the obsed values fit the predicted
}
...
real<lower=0> ind_v_eff_isl_year[n_yi];
real<lower=0> dem_v_eff;
real<lower=0> ind_v_eff;
real<lower=0> tot;
...
vector[n_yi]  E_lambda;

E_lambda= B[1] - log(2) + YI;
 // the covariance between r0 and gamma, islands
...
ind_v         = (exp(sigma2_I)-1)*exp(2*(B[1]-log(2)) + sigma2_I);
spatio_temp_v = (exp(sigma2_YI)-1)*exp(2*(B[1]-log(2)) + sigma2_YI);
temp_v        = (exp(sigma2_Y)-1)*exp(2*(B[1]-log(2)) + sigma2_Y);
spatial_v     = (exp(sigma2_Isl[1])-1)*exp(2*(B[1]-log(2)) + sigma2_Isl[1]);
dem_v         = exp(B[1]-log(2)) + exp(B[1]-log(2))^2/phi;

den_v = (B[2]^2 + sigma2_Isl[2])*v_n;

for(i in 1:n_yi){
  dem_v_eff_isl_year[i] = dem_v/n_ind_isl_year[i];
  ind_v_eff_isl_year[i] = ind_v/n_ind_isl_year[i];
}

dem_v_eff= sum(dem_v_eff_isl_year)/n_yi;
ind_v_eff= sum(ind_v_eff_isl_year)/n_yi;

tot = spatio_temp_v + temp_v + spatial_v + dem_v_eff + den_v + ind_v_eff;

//Proportions
p_ind_v =ind_v_eff/tot; 
